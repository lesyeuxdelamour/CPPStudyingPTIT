...
class HoaDon
{
    ...
    public:
        ...
        friend istream& operator >> (istream &in, HoaDon &x)
        {
            ...
            x.profit = (MH[x.IDmh].getsell() - MH[x.IDmh].getbuy()) * x.number;
            return in;      
        }
        ...
        friend ostream& operator << (ostream &out, HoaDon x)
        {
			...
			out << MH[x.IDmh].getname() << " ";
			out << x.number << " " << MH[x.IDmh].getsell() * x.number << " " << x.profit << endl;
            return out;
        }
